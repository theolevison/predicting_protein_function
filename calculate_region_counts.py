...
domain_list = "G:/diss/final_domains_E.dat"
...
with open(domain_list, 'r') as dl:
    ...
    for row in domreader:
        ...
        # print(row[5])
        if row[6].startswith('PF'):
            prots[row[0]]['domains'] += int(row[8])-int(row[7])
        if row[6].startswith('mobidb-lite'):
            prots[row[0]]['disorder'] += int(row[8])-int(row[7])
        if row[6].startswith('CoiledCoil'):
            prots[row[0]]['coiledcoil'] += int(row[8])-int(row[7])
        if row[6].startswith('LowComplexity'):
            prots[row[0]]['lowcomplexity'] += int(row[8])-int(row[7])
