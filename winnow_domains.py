...
def handle_cache(data):
    ...
    for idx, line in enumerate(data):
        ...
        if len(entries) < 9:
            continue
        len1 = int(entries[8])-int(entries[7])
        dom_type = return_dom_type(entries[6])
        if len(entries) == 10:
            prot_length = entries[9]
        for idx2, line2 in enumerate(data):
            ...
            if len(entries2) < 9:
                continue
            len2 = int(entries2[8])-int(entries2[7])
            dom_type2 = return_dom_type(entries2[6])

            if int(entries[7]) in range(int(entries2[7]), int(entries2[8])+1) \
               or int(entries[8]) in \
               range(int(entries2[7]), int(entries2[8])+1):
