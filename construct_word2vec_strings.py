...
def handle_cache(data):
    ...
    sorted_data = sorted(splits, key=lambda x: x[7])
    protein_length = int(sorted_data[0][9])
    ...
    previous_start = 0
    domain_size = len(list(takewhile(lambda x: x[6].startswith("PF"), sorted_data)))
    domains_and_gaps = []
    for idx, domain in enumerate(takewhile(lambda x: x[6].startswith("PF"), sorted_data)):
        if int(domain[7]) > previous_start+1:
            if ((int(domain[7])-1) - (previous_start+1)) > 20:
                gap_id = return_gap_name(previous_start+1, int(domain[7])-1)
                domains_and_gaps.append([gap_id, domain[0], previous_start+1, int(domain[7])-1])
            domains_and_gaps.append([domain[6], domain[0], domain[7], domain[8]])
        else:
            domains_and_gaps.append([domain[6], domain[0], domain[7], domain[8]])
        if idx+1 == domain_size:
            if int(domain[8]) < protein_length:
                if (protein_length - int(domain[8])) > 20:
                    gap_id = return_gap_name(int(domain[8]), protein_length)
                    domains_and_gaps.append([gap_id, domain[0], int(domain[8]), protein_length])
        previous_start = int(domain[9])

    #print(domains_and_gaps)
    for idx, region in enumerate(domains_and_gaps):
        if not region[0].startswith("PF"):
            for domain in sorted_data:
                if not domain[6].startswith("PF"):
                    if int(domain[7]) in range(int(region[2]), int(region[3])):
                        #print(domain)
                        if "G" in domains_and_gaps[idx][0][0]:
                            if domain[6].startswith("mobidb-lite"):
                                domains_and_gaps[idx][0] = "D"+domains_and_gaps[idx][0]
                            elif domain[6].startswith("Low"):
                                domains_and_gaps[idx][0] = "L"+domains_and_gaps[idx][0]
                            elif domain[6].startswith("Coiled"):
